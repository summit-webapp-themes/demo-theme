...
const FixedFiltersLayout = ({ children, page }: {
  ...
  const odTc = router.isReady ? getQueryParam(router.query.OdTc) : undefined;
  const odChr = router.isReady ? getQueryParam(router.query.OdChr) : undefined;
  ...
  const [moveToSelectedProducts, setMoveToSelectedProducts] = useState<any[]>([]);
  const {
    isLoading,
    productsData,
    productsCount,
  ...
  const [moveToFromPage, setMoveToFromPage] = useState<'detail' | 'listing'>('listing');
  ...
  const handleApplyFilters = async (filters?: any, isPageIncrementValue?: boolean) => {
    // If filters is an Event (like a React click event), ignore it
    const cleanFilters = (filters && (filters.target || filters.preventDefault || filters.nativeEvent)) ? undefined : filters;

    // handleResetInputs();
    if ((cleanFilters?.customer?.value !== selectedCustomerCode?.value) && (cleanFilters?.selectedScope?.value !== selectedScope?.value)) {
      setSelectedProducts([]);
      dispatch(setSelectAllProducts(false));
    } else if ((cleanFilters?.customer?.value === selectedCustomerCode?.value) && (cleanFilters?.selectedScope?.value !== selectedScope?.value)) {
      setSelectedProducts([]);
      dispatch(setSelectAllProducts(false));
    } else if ((cleanFilters?.customer?.value !== selectedCustomerCode?.value) && (cleanFilters?.selectedScope?.value === selectedScope?.value)) {
      setSelectedProducts([]);
    ...
    if (cleanFilters?.customer?.value !== selectedCustomerCode?.value) {
      fetchCartData('CT', cleanFilters?.customer?.value, companyCode?.value);
      fetchCartData('SCT', cleanFilters?.customer?.value, companyCode?.value);
    }
    selectedCustomerCode?.value !== "" && dispatch(setHideFiltersOnFirstLoad(false));

    const shouldResetForVoucher = cleanFilters?.selectedScope?.value === 'Voucher';
    const resetVoucherFilters = {
      ...
      voucherOdNoRange: cleanFilters?.voucherOdNoRange ?? [0, 0],
    ...
    const finalPayload = shouldResetForVoucher ? { ...payload, ...resetVoucherFilters, ...cleanFilters } : payload;
  ...
  useEffect(() => {
    ...
    // setSelectedProducts([]);
    dispatch(setMetalRateSidebar(false));
    // Note: Do NOT close userDefaultSidebar here — it's controlled independently via the profile icon
    setMoveToSidebar(false);
    setCustomiseSidebar(false);
    setMetalRateSidebar(false);
    setRefreshRateSidebar(false);
  ...
  if (!ready || !router.isReady) return null;
    ...
    <div className="row m-0" style={{ backgroundColor: page === "product-listing" ? '#FCFCFC' : '#FFFFFF', minHeight: '100vh', height: 'fit-content' }}>
      {page === 'product-listing' && (
        <FallbackLayoutSidebar
          ...
          stockIdRange={stockIdRange}
          setStockIdRange={setStockIdRange}
          ...
          dynamicConfig={dynamicFilterConfig}
          fetchDynamicConfig={fetchDynamicConfig}
        />
      )}
      <KCProductListing
