...
type Props = {
  ...
  moveToFromPage: 'detail' | 'listing'
  setMoveToFromPage: any;
  odTc: string | undefined;
  odChr: string | undefined;
...
export default function FallbackLayoutModals(p: Props) {
  ...
  const useUrlScope = p.moveToFromPage === 'detail' && p.odTc && p.odChr ? true : false;

  const getEffectiveScope = () => {
    // 1. PRIORITY: odTc + odChr mapping (new logic)
    if (useUrlScope) {
      if (p.odTc === 'PL') return 'PDCM Design Bank';

      if (p.odTc === 'JQB' && p.odChr === 'IQT') return 'Stock';
      if (p.odTc === 'JQB' && p.odChr === 'SCT') return 'Stock Cart';

      if (p.odTc === 'QT' && p.odChr && ['CT', 'QT'].includes(p.odChr)) return 'Cart';

      if (p.odTc === 'JSA' && p.odChr === 'JSA') return 'JSA';

      if (p.odTc === 'JMV') return 'JMV';

      if (p.odTc === 'JMI' && p.odChr === 'JMI') return 'Voucher';
    }

    // 2. fallback: existing system
    if (p.currentScope === 'Voucher' && p.selectedVoucherType?.value === 'JSA') return 'JSA';

    if (p.currentScope === 'Voucher' && p.selectedVoucherType?.value === 'JMV') return 'JMV';

    if (p.currentScope === 'Voucher' && p.selectedVoucherType?.value !== 'JMI') return 'Cart';

    if (p.currentScope === 'Stock' && p.selectedStockType?.value === 'Blocked') return 'Blocked';

    return p.currentScope;
  };

  const dynamicScope = useMemo(() => getEffectiveScope(), [
    p.odTc,
    p.odChr,
    p.currentScope,
    p.selectedVoucherType?.value,
    p.selectedStockType?.value,
    p.moveToFromPage,
  ]);
  ...
  return (
    <>
      ...
      <MoveToSidebarWrapper
        ...
        setSelectedProducts={p.moveToFromPage === 'listing' ? p.setSelectedProducts : p.setMoveToSelectedProducts}
        handleApplyFilters={p.handleApplyFilters}
        ...
        odTc={p.odTc}
        odChr={p.odChr}
        moveToSelectedProducts={p.moveToSelectedProducts}
        setMoveToSelectedProducts={p.setMoveToSelectedProducts}
        moveToFromPage={p.moveToFromPage}
      />
      {p.customiseSidebar && (
        <CustomiseSidebar
          ...
          activeScope={p.currentScope}
          selectedCustomerCode={p.selectedCustomerCode}
          selectedProducts={p.moveToFromPage === 'listing' ? p.selectedProducts : p.moveToSelectedProducts}
      ...
      <MoveToModal
        ...
        handleClose={p.handleClose}
        voucherType={(dynamicScope === 'PDCM Design Bank' || dynamicScope === 'Stock') ? p.selectedCartType : p.selectedModalVoucherType}
        ...
        moveToFromPage={p.moveToFromPage}
        setMoveToFromPage={p.setMoveToFromPage}
      />
      {p.syncStockModal && (
        <SyncStockModal
