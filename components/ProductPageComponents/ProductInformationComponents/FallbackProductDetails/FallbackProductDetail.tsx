...
import styles2 from '../../../../styles/addon-styles/productPageV2Components.module.scss';
  ...
  const bomDetails = [
    {
      ...
      tableData: metalDetails?.map((item: any) => ({
        ...
        rate: item.OrSalRt ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OrSalRt, true) : '-',
        value: item.OrSalVal ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OrSalVal, true) : '-',
        salVal: item.OrSalVal ?? 0,
      })),
    },
    {
    ...
    },
    {
      ...
      tableData: colorStoneDetails?.map((item: any) => ({
        ...
        rate: item.OrSalVal ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OrSalVal, true) : '-',
        value: item.OrSalVal ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OrSalVal, true) : '-',
        salVal: item.OrSalVal ?? 0,
      })),
    },
    {
      ...
      tableData: accessoriesDetails?.map((item: any) => ({
        ...
        weight: item.OrWt ? `${item.OrWt.toFixed(2)} g` : '-',
        rate: item.OrSalRt
          ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OrSalRt, true)
          : '-',
        value: item.OrSalVal
          ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OrSalVal, true)
          : '-',
        salVal: item.OrSalVal ?? 0,
      })),
    },
    {
      ...
      tableData: labourDetails?.map((item: any) => ({
        ...
        subCd: item.OlScd || '-',
        // qtyOrWt: item.OlQw || '',
        rate: item.OlSalRt ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OlSalRt, true) : '-',
        value: item.OlSalVal ? formatPriceBasedOnCurrency(selectedCurrency?.symbol, item.OlSalVal, true) : '-',
        salVal: item.OlSalVal ?? 0,
      })),
    },
  ];
    ...
    <div className='row'>
                  ...
                  <thead>
                    <tr className={styles.productTableHeader}>
                      {detail.tableHeaders.map((header: string, index: number) => (
                        <th key={`detail-table-header-${header}-${index}`} className={header === 'Rate' || header === 'Value' ? ' text-end pe-5 pe-xxl-3' : ''}>{header}</th>
                      ))}
                      {detail.tableHeaders?.length < 5 && <th style={{ width: '6%' }}></th>}
                    </tr>
                  </thead>
                  <tbody className={styles.productTableBody}>
                    {detail.tableData.map(({ salVal, ...item }: any, index: number) => (
                      <tr key={`detail-table-item-${index}`} className={`${salVal ? styles.productTableBodyRow : styles.productTableBodyRowHighlight}`} style={{ position: 'relative' }}>
                        {detail.keys.map((key: string, keyIndex: number) => (
                          <td key={`item-table-data-${index}-${keyIndex}`} className={`${key === 'rate' || key === 'value' ? ' text-end pe-5 pe-xxl-3' : ''}${['qty', 'weight', 'size1', 'size2', 'carats'].includes(key) ? ' lato-regular' : ''}`}>{t(item?.[key])}</td>
                        ))}
                        {detail.tableHeaders?.length < 5 && <td></td>}
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className='col-md-5 col-lg-4 pt-1 pb-3'>
        <div className='d-flex justify-content-between align-items-center'>
          ...
          <button
            className={` btn btn-sm ${styles2.fetchPriceButton}`}
            onClick={() => updateProductInstructions(productDetailData?.OdIdNo, stmpInst, szInst, dmPrdInst, spcRem, subRem)}
          // disabled={
        ...
        <Accordion defaultActiveKey={['0', '1', '2', '3', '4']} alwaysOpen className='productDetailsAccordian'>
