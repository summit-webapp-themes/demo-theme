...
export default function FallbackProductInformation({
  ...
  const { goldRate, platinumRate, palladiumRate, silverRate, companyCode } = useSelector(KCFromStore);
  const {
    designSizes,
    attributesData,
  ...
  const refreshPrice = async () => {
    ...
    try {
      ...
      if (productPriceResponse?.status === 200) {
        ...
        if (odChr === 'CT' || 'SCT') {
          fetchCartData(odChr, customerCode?.value, companyCode?.value);
        };
    ...
    <div className="bg-white h-100" style={{ border: '1px solid #DADADA', borderRadius: '10px', padding: '12px 28px' }}>
      ...
      {attributesData?.length > 0 && !loadingAttributeData ? (
        attributesData.map((attribute: any, index: number) => {
          ...
          // show all options if OdChr is CT, else only selected value
          const valuesToRender = odChr
            ? attributeValues.filter((val: any) => {
                ...
                // return val.id === selectedValueMap[attribute.field_name];
              })
      ...
      {(odTc === 'PL' || odChr === 'IQT') && (
        ...
        >
          Note: Product customization, including metal grade, diamond, color stone, size, and other specifications, can only be modified after the item has been added to the cart.
        </p>
      )}
      <div className={` d-flex mt-2 align-items-center flex-wrap gap-2 ${(odTc === 'PL') ? 'justify-content-between' : 'justify-content-end'}`}>
        {/* Quantity Controls */}
        {(odTc === 'PL') && ( 
          <div
