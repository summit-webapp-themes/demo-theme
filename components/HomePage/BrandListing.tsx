...
import useTopBrand from '../../hooks/HomePageHooks/TopBrandsHooks';
...
const BrandListing = () => {
  const { brandListing } = useTopBrand();
            ...
            <div key={`${j}`} className="card-wrapper mx-auto " style={{ width: '96%' }}>
              <Link href={val?.url} className="banner-title text-white text-capitalize ls-25 homecategory_btnlink">
                <div className="category-banner banner banner-fixed ">
                  <Image loader={imageLoader} src={val?.image} alt="Picture of the author" width={130} height={130} />
                </div>
              </Link>
            </div>
          ))}
