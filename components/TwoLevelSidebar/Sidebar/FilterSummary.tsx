...
const FilterSummary = ({ filters }: { filters: any }) => {
  ...
  const formatArray = (arr: any[]) =>
    Array.isArray(arr) ? arr.map((item) => item.label).join(", ") : "";

  const filterItems = [
    { label: "Customer Code", value: filters?.customer?.label || "" },
    { label: "Source Type", value: filters?.source?.label || "" },
    { label: "Sales Category", value: formatArray(filters?.category) },
    { label: "Design Category", value: formatArray(filters?.designColor) },
    {
      label: "Price Range",
    ...
    },
    {
      ...
      value: filters?.diamond
        ? `${filters.diamond[0]} - ${filters.diamond[1]}`
        : "",
    },
    {
      label: "Gross Wt",
      value: filters?.grossWtRange
        ? `${filters.grossWtRange[0]} - ${filters.grossWtRange[1]}`
        : "",
    },
      ...
      >
        {filterItems.map((item) =>
          item.value ? (
            ...
            >
              <strong style={{ color: "#a67c52" }}>{item.label}:</strong>{" "}
