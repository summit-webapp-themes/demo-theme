...
const EditAddressModalFields = ({ showAddress, stateList,emptyAddressFields, handleEditAddressChange, setShow,cityList, handlePostAddress }: any) => {
 console.log(emptyAddressFields.includes(showAddress.showAddress?.postal_code),emptyAddressFields,"bb")
  return (
    <>
      <Form>
        <Form.Group className="mb-3" controlId="name">
          <Form.Label>Name<span className='mandatoryField'>*</span></Form.Label>
          <Form.Control isInvalid={emptyAddressFields.includes("name")} type="text" placeholder="Name" name="name" defaultValue={showAddress?.name} onChange={handleEditAddressChange} />
        </Form.Group>
        ...
        <Form.Group className="mb-3" controlId="country">
          ...
          <Form.Select aria-label="country" name="country" value={showAddress?.country} onChange={handleEditAddressChange}  isInvalid={emptyAddressFields.includes("country")}>
      ...
      <Button className={style.submitAddress} onClick={() => handlePostAddress(showAddress?.address_type,setShow)}>
        Save Address
      </Button>
    </>
  );
