...
export default function KCBreadCrumbs() {
  ...
  const { scope, customer } = useSelector(KCFromStore);
    ...
    <nav aria-label="breadcrumb">
      <ol className="breadcrumb m-0">
        <li className={styles.breadCrumbItem}>
          <Link href={`/product-category?page=1&currency=${selectedCurrency?.value}&scope=${scope?.value}&cmcd=${customer?.value}`} legacyBehavior>
            <a>{t('back')}</a>
          </Link>
        </li>
